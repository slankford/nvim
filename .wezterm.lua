...
local function split_resize_or_send(direction, delta, mod_keys)
	return wezterm.action_callback(function(window, pane)
		if is_vim(pane) then
			...
			window:perform_action(act.SendKey({ key = key_for_dir[direction], mods = mod_keys }), pane)
		else
...
config.keys = {
	...
	-- Pain resizes
	{ key = "LeftArrow", mods = pane_resizes_mods, action = split_resize_or_send("Left", 3, split_mods) },
	{ key = "RightArrow", mods = pane_resizes_mods, action = split_resize_or_send("Right", 3, split_mods) },
	{ key = "UpArrow", mods = pane_resizes_mods, action = split_resize_or_send("Up", 2, split_mods) },
	{ key = "DownArrow", mods = pane_resizes_mods, action = split_resize_or_send("Down", 2, split_mods) },
